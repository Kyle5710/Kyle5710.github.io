...
function squareFractal(x, y, len) {
  if (len > 50) {
    ...
    //applies to all squares on the screen
    push();
    translate(x, y);
    rotate(radians(frameCount));
    square(0, 0, len);
    pop();
    //stop applying stuff to all squares on the screen

    squareFractal(x - len / 2, y - len / 2, len / 2); //top left corner
    squareFractal(x - len / 2, y + len / 2, len / 2); //bottom left corner
    squareFractal(x + len / 2, y - len / 2, len / 2); //top right corner
    squareFractal(x + len / 2, y + len / 2, len / 2); //bottom right corner
  }
}